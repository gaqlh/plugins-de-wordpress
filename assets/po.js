...
(function(){
    ...
    const configs = [
      {
        ...
        onClear: (sel)=> { sel.value = ''; sel.dispatchEvent(new Event('change', { bubbles:true })); }
      },
      {
        id: 'vscPoStockFilter',
        isActive: (sel)=> !!sel.value,
      ...
      },
      {
        ...
        isActive: (sel)=> Number(sel.value || 0) > 0,
        onClear: (sel)=> {
          sel.value = '';
          // mantener el comportamiento actual (este filtro recarga al cambiar)
          sel.dispatchEvent(new Event('change', { bubbles:true }));
        }
      }
    ];
  ...
  function renderSupplierFilter(){
    const sel = $('vscPoSupplierFilter');
    if (!sel) return;
    
    const keep = String(state.supplier_id || 0);
    sel.innerHTML = '';
    
    const ph = document.createElement('option');
    ph.value = '';
    ph.textContent = 'Filtrar por proveedores';
    ph.disabled = true;
    ph.hidden = true;
    sel.appendChild(ph);
  ...
  async function loadItems(){
    ...
    try {
      ...
      if (state.supplier_id && Number(state.supplier_id) > 0) params.set('supplier_id', String(state.supplier_id));
  ...
  function bindUI(){
    ...
    const syncFiltersFromUI = ()=>{
      ...
      state.supplier_id = Number($('vscPoSupplierFilter').value || 0);
    ...
    $('vscPoSupplierFilter').addEventListener('change', applyFilters);
