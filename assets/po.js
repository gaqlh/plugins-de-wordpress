...
(function(){
  ...
  // Hook para refrescar los botones de limpiar (X) cuando el filtro es multi-select
  let updateClearButtons = ()=>{};
  ...
  function updateStats() {
    ...
    if (selectedEl) selectedEl.textContent = `${state.select.size} seleccionados`;
    if (dirtyEl) dirtyEl.textContent = `${state.dirtyInventory.size} cambios pendientes`;
  }

  function updateStockFilterUI(){
    const sel = $('vscPoStockFilter');
    if (!sel) return;
    sel.classList.remove('vsc-po-stockfilter--out','vsc-po-stockfilter--low','vsc-po-stockfilter--good');
    const v = sel.value;
  ...
  function setupClearableFilters(){
    ...
    const configs = [
      {
        id: 'vscPoCategoryFilter',
        // Multi-select: activo si hay al menos 1 categoría seleccionada (UI)
        isActive: ()=> Array.isArray(state.category_ids_ui) && state.category_ids_ui.length > 0,
        onClear: ()=> {
          state.category_ids_ui = [];
          state.category_ids = [];
      ...
      },
{
      ...
      },
      {
        ...
        isActive: ()=> Array.isArray(state.supplier_ids_ui) && state.supplier_ids_ui.length > 0,
        onClear: ()=> {
          state.supplier_ids_ui = [];
          state.supplier_ids = [];
    ...
    // expone un refresco manual (útil para multi-select con checkboxes)
    updateClearButtons = ()=>{ updaters.forEach(fn=>{ try{ fn(); }catch(e){} }); };
  }
  ...
  function stockLabel(s){
    switch(s){
      ...
      case 'low': return 'Bajo mínimo';
      case 'good': return 'Buen stock';
      default: return '';
    }
  }
  ...
  function canSelect(item){
    return item.manage_stock && item.min_stock !== null && item.has_valid_supplier;
  }
  ...
  function renderPagination(containerId){
    ...
    prev.disabled = p <= 1;
    prev.onclick = ()=>{ state.page = Math.max(1, p-1); loadItems(); };

    const next = document.createElement('button');
    next.className = 'vsc-po-btn';
    next.textContent = 'Siguiente →';
    next.disabled = p >= pages;
    next.onclick = ()=>{ state.page = Math.min(pages, p+1); loadItems(); };
  ...
  function renderTable(){
    ...
    if (state.items.length === 0) {
      tbody.innerHTML = '<tr><td colspan="11" class="vsc-po-loading">No se encontraron productos con los filtros actuales.</td></tr>';
    ...
    state.items.forEach(item => {
      ...
      cb.title = canSelect(item) ? 'Seleccionar para pedido' : 'Requiere stock mínimo y proveedor válido';
      cb.onchange = ()=> {
      ...
      tdImg.innerHTML = item.img ? `<img src="${item.img}" class="vsc-po-thumb" alt="${item.name}" />` : '<div class="vsc-po-thumb"></div>';
      ...
      tdStock.classList.add('vsc-po-stockcell--' + (item.stock_status || 'pending'));
      tdStock.title = stockLabel(item.stock_status);
      tdStock.textContent = (item.stock_quantity === null ? '-' : item.stock_quantity);
      ...
      inpQty.min = '0';
      inpQty.disabled = !state.editStockEnabled.has(item.id) || !item.manage_stock;
      inpQty.placeholder = '-';
      if (Object.prototype.hasOwnProperty.call(dirtyPatch, 'stock_quantity')) {
        inpQty.classList.add('vsc-po-input--dirty');
      }
      inpQty.oninput = ()=>{
        const value = inpQty.value === '' ? null : Math.max(0, parseInt(inpQty.value) || 0);
        markDirty(item.id, { stock_quantity: value });
        const dpQty = state.dirtyInventory.get(item.id) || {};
        if (Object.prototype.hasOwnProperty.call(dpQty, 'stock_quantity')) inpQty.classList.add('vsc-po-input--dirty');
      ...
      inpMin.oninput = ()=>{
        const value = inpMin.value === '' ? null : Math.max(0, parseInt(inpMin.value) || 0);
        markDirty(item.id, { min_stock: value });
        const dpMin = state.dirtyInventory.get(item.id) || {};
        if (Object.prototype.hasOwnProperty.call(dpMin, 'min_stock')) inpMin.classList.add('vsc-po-input--dirty');
      ...
      if (item.has_valid_supplier && Array.isArray(item.supplier_choices) && item.supplier_choices.length) {
        if (!item.current_supplier_id) item.current_supplier_id = item.best_supplier_id || item.supplier_choices[0].supplier_id;
        if (item.current_wholesale === undefined || item.current_wholesale === null) item.current_wholesale = item.best_wholesale;

        const nameSpan = document.createElement('span');
        nameSpan.className = 'vsc-po-suppliername';
        const supObj = state.suppliers.find(s=>Number(s.id)===Number(item.current_supplier_id));
        nameSpan.textContent = supObj ? supObj.name : ('Proveedor ' + item.current_supplier_id);
      ...
      const initialWholesale = (item.current_wholesale !== undefined && item.current_wholesale !== null) ? item.current_wholesale : item.best_wholesale;
    ...
    if (selectAllCb) {
      ...
      const allSelected = selectableItems.length > 0 && selectableItems.every(item => state.select.has(item.id));
      selectAllCb.checked = allSelected;
      selectAllCb.indeterminate = !allSelected && selectableItems.some(item => state.select.has(item.id));
  ...
  function computeClientStatus(item){
    if (!item.manage_stock || item.min_stock === null || item.stock_quantity === null) return 'pending';
  ...
  function setDirtyField(product_id, field, value){
    ...
    const baseVal = (field === 'manage_stock') ? !!base.manage_stock : normInt(base[field]);
  ...
  function renderCategoryFilter(){
    const wrap = $('vscPoCategoryFilter');
    const btn = $('vscPoCategoryBtn');
    const panel = $('vscPoCategoryPanel');
    if (!wrap || !btn || !panel) return;

    // Normaliza selección UI
    const uniq = Array.from(new Set((state.category_ids_ui || []).map(n => Number(n)).filter(n => n > 0)));
    state.category_ids_ui = uniq;

    const getLabel = ()=>{
      if (!state.categories || state.categories.length === 0) return 'Filtrar por categoría';
      if (state.category_ids_ui.length === 0) return 'Filtrar por categoría';
      if (state.category_ids_ui.length === 1) {
        const c = state.categories.find(x => Number(x.id) === Number(state.category_ids_ui[0]));
        return c ? c.name : 'Filtrar por categoría';
      }
      return `Categorías (${state.category_ids_ui.length})`;
    };
    ...
    if (!state.categories || state.categories.length === 0) {
      panel.innerHTML = '<div class="vsc-po-multiselect__empty">No hay categorías</div>';
      return;
    }
    ...
    state.categories.forEach(c=>{
      ...
      cb.checked = state.category_ids_ui.includes(Number(c.id));
      cb.addEventListener('change', ()=>{
        const id = Number(c.id);
        if (cb.checked) {
          if (!state.category_ids_ui.includes(id)) state.category_ids_ui.push(id);
        } else {
          state.category_ids_ui = state.category_ids_ui.filter(x => Number(x) !== id);
        }
        // NO aplicar aquí; se aplica con Buscar
        renderCategoryFilter();
        updateClearButtons();
      });
      const span = document.createElement('span');
      const countTxt = (c.count !== undefined && c.count !== null) ? ` (${c.count})` : '';
      span.textContent = `${c.name}${countTxt}`;
      row.appendChild(cb);
      row.appendChild(span);
      panel.appendChild(row);
    });

    panel.setAttribute('aria-hidden', wrap.classList.contains('is-open') ? 'false' : 'true');
  }
  ...
  function renderSupplierFilter(){
    ...
    // Normaliza selección
    const uniq = Array.from(new Set((state.supplier_ids_ui || []).map(n => Number(n)).filter(n => n > 0)));
    state.supplier_ids_ui = uniq;

    const getLabel = ()=>{
      if (!state.suppliers || state.suppliers.length === 0) return 'Filtrar por proveedores';
      if (state.supplier_ids_ui.length === 0) return 'Filtrar por proveedores';
      if (state.supplier_ids_ui.length === 1) {
        const s = state.suppliers.find(x => Number(x.id) === Number(state.supplier_ids_ui[0]));
        return s ? s.name : 'Filtrar por proveedores';
      }
      return `Proveedores (${state.supplier_ids_ui.length})`;
    };
    ...
    if (!state.suppliers || state.suppliers.length === 0) {
      panel.innerHTML = '<div class="vsc-po-multiselect__empty">No hay proveedores</div>';
      return;
    }
    ...
    state.suppliers.forEach(s=>{
      ...
      cb.checked = state.supplier_ids_ui.includes(Number(s.id));
      cb.addEventListener('change', ()=>{
        const id = Number(s.id);
        if (cb.checked) {
          if (!state.supplier_ids_ui.includes(id)) state.supplier_ids_ui.push(id);
        } else {
          state.supplier_ids_ui = state.supplier_ids_ui.filter(x => Number(x) !== id);
        }
        // NO aplicar aquí; se aplica con Buscar
        renderSupplierFilter();
        updateClearButtons();
  ...
  async function loadItems(){
    ...
    try {
      ...
      if (Array.isArray(state.category_ids) && state.category_ids.length > 0) {
        state.category_ids.forEach(cid => {
          if (Number(cid) > 0) params.append('category_ids[]', String(cid));
        });
      }
      ...
      state.items = (data.items || []).map(it=>{
        ...
        if (it.current_supplier_id === undefined || it.current_supplier_id === null) it.current_supplier_id = it.best_supplier_id;
        if (it.current_wholesale === undefined || it.current_wholesale === null) it.current_wholesale = it.best_wholesale;
        return it;
      });
      ...
      state.items.forEach(it=>{
        if (!state.originalInventory.has(it.id)) {
          state.originalInventory.set(it.id, {
            ...
            stock_quantity: (it.stock_quantity === null || it.stock_quantity === undefined) ? null : Number(it.stock_quantity),
            min_stock: (it.min_stock === null || it.min_stock === undefined) ? null : Number(it.min_stock),
          });
        }
      });
  ...
  async function saveInventory(){
    ...
    try{
      const data = await api('/inventory/bulk', { method:'POST', body: JSON.stringify({ updates }) });
      toast(`Cambios guardados: ${data.updated.length} productos actualizados.`, 'ok');
  ...
  async function openChooseSupplierModal(item){
    ...
    try {
      if (!Array.isArray(item.supplier_choices) || item.supplier_choices.length === 0) {
      ...
      sel.value = String(item.current_supplier_id || item.best_supplier_id || item.supplier_choices[0].supplier_id);
      row.appendChild(sel);
  ...
  async function saveAssignModal(){
    ...
    try {
      if (state.modal.mode === 'choose') {
        ...
        const sid = Number(($('vscPoModalChooseSupplier') && $('vscPoModalChooseSupplier').value) ? $('vscPoModalChooseSupplier').value : 0);
        if (!pid || !sid) {
        ...
        state.items = state.items.map(it=>{
          ...
          const chosen = (Array.isArray(it.supplier_choices) ? it.supplier_choices.find(x=>Number(x.supplier_id)===sid) : null);
          return Object.assign({}, it, {
            current_supplier_id: sid,
            current_wholesale: chosen ? Number(chosen.wholesale_cost) : it.best_wholesale
          });
        });
        ...
        const data = await api('/assign-supplier', {
          method:'POST',
          body: JSON.stringify({ product_id: pid, supplier_id: sid, wholesale_cost: w, sku: skuEl ? sku : undefined })
        });
        ...
        state.items = state.items.map(it=>{
          ...
          return Object.assign({}, it, {
            ...
            supplier_choices: data.supplier_choices || it.supplier_choices || [],
  ...
  async function generateOrders(){
    ...
    if (invalid) {
      toast('Hay productos seleccionados que no cumplen requisitos (stock mínimo y proveedor válido).', 'err');
      return;
    }
    ...
    selected.forEach(it=>{
      ...
      bySupplier.get(sid).push({
        ...
        sku: it.sku,
        name: it.name,
        wholesale_cost: ((it.current_wholesale !== undefined && it.current_wholesale !== null) ? it.current_wholesale : it.best_wholesale),
        quantity: 1,
        supplier_id: sid
      });
    });
    ...
    state.orders = Array.from(bySupplier.entries()).map(([sid, items])=>{
      ...
      return { supplier_id: sid, supplier_name: sup ? sup.name : ('Proveedor ' + sid), items };
    });

    renderOrders();
    $('vscPoStep2').style.display = 'block';
    toast(`Pedido generado con ${selected.length} productos en ${state.orders.length} proveedores.`, 'ok');
    hideLoading();
  }

  async function fetchValidSuppliers(product_id){
    const data = await api('/item-suppliers?product_id=' + encodeURIComponent(product_id), { method:'GET' });
  ...
  function renderOrders(){
    ...
    if (!state.orders || state.orders.length === 0) {
      wrap.innerHTML = '<div class="vsc-po-muted">No hay pedido generado.</div>';
      if (summary) summary.textContent = '';
      return;
    }

    const totalItems = state.orders.reduce((sum, o) => sum + o.items.length, 0);
    const totalValue = state.orders.reduce((sum, o) => sum + calcOrderTotal(o), 0);
    ...
    state.orders.forEach(order=>{
      ...
      order.items.forEach(it=>{
        ...
        const tdW = document.createElement('td'); tdW.className='vsc-po-col--num'; tdW.textContent = '$' + money(it.wholesale_cost);
        ...
        fetchValidSuppliers(it.product_id).then(list=>{
          sel.innerHTML = '';
          list.forEach(s=>{
            const o = document.createElement('option');
        ...
        }).catch(()=>{ sel.innerHTML = `<option value="${it.supplier_id}">Proveedor ${it.supplier_id}</option>`; });
        ...
        sel.onchange = async ()=>{
          ...
          if (chosen) {
            ...
            moveItemToSupplier(it.product_id, order.supplier_id, newSid, chosen);
          }
        };

        const tdQty = document.createElement('td'); tdQty.className='vsc-po-col--num';
        const inp = document.createElement('input');
        ...
        const tdSub = document.createElement('td'); tdSub.className='vsc-po-col--num';
        tdSub.textContent = '$' + money(it.quantity * it.wholesale_cost);
  ...
  function calcOrderTotal(order){
    return (order.items || []).reduce((sum, it)=> sum + (Number(it.quantity||1) * Number(it.wholesale_cost||0)), 0);
  }

  function renderOrderTotals(){
    if (!state.orders) return;
    state.orders.forEach(o=>{
      const el = document.querySelector(`.vsc-po-total[data-supplier-id="${o.supplier_id}"]`);
  ...
  function moveItemToSupplier(product_id, fromSupplier, toSupplier, chosen){
    ...
    if (!to) {
      ...
      to = { supplier_id: toSupplier, supplier_name: sup ? sup.name : ('Proveedor ' + toSupplier), items: [] };
  ...
  function exportCsvSupplier(supplier_id){
    ...
    order.items.forEach(it=>{
      rows.push([it.sku, it.name, money(it.wholesale_cost), it.quantity, money(it.quantity*it.wholesale_cost)]);
    });
    ...
    download('pedido_' + order.supplier_name.replace(/[^a-z0-9]/gi, '_') + '.csv', csv);
  }
  ...
  function exportCsvAll(){
    ...
    rows.push([]);
    rows.push(['Proveedor','SKU','Producto','Precio Mayorista','Cantidad','Subtotal']);
    
    state.orders.forEach(order=>{
      order.items.forEach(it=>{
        rows.push([order.supplier_name, it.sku, it.name, money(it.wholesale_cost), it.quantity, money(it.quantity*it.wholesale_cost)]);
      });
    });
    ...
    const csv = rows.map(r=>r.map(csvEscape).join(',')).join('\n');
    download('pedido_completo_' + new Date().toISOString().split('T')[0] + '.csv', csv);
  }
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          `).join('') : '<p>No hay pedido generado.</p>'}
        </body>
      </html>
    `);
  ...
  function bindUI(){
    ...
    const syncFiltersFromUI = ()=>{
      // Nota: categorías/proveedores multi-select se aplican solo con el botón "Buscar"
    ...
    $('vscPoRefresh').onclick = ()=>{
      ...
      // Aplicar buffers de multi-select
      state.supplier_ids = Array.from(new Set((state.supplier_ids_ui || []).map(n=>Number(n)).filter(n=>n>0)));
      state.category_ids = Array.from(new Set((state.category_ids_ui || []).map(n=>Number(n)).filter(n=>n>0)));
    ...
    // Auto-aplicar: solo stock y per_page. Categorías/Proveedores se aplican con "Buscar".
    ...
    // Multi-select categorías: dropdown con checkboxes
